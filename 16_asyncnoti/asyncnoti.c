...
static void timer_function(unsigned long arg)
{
    ...
    if (atomic_read(&dev->ifreleasekey)) /*按下按键*/
    {
        if (dev->async_queue)
        {
            kill_fasync(&dev->async_queue, SIGIO, POLL_IN); /*可以进行输入输出操作*/
        }
    }
#if 0
    /*唤醒进程*/
...
static int imx6ullirq_fasync(int fd, struct file *filp, int on)
{
    struct blockio_dev *dev = (struct blockio_dev *)filp->private_data;
...
{
    return imx6ullirq_fasync(-1, filp, 0); /*删除异步通知*/
}
...
static int __init blockio_init(void) /*入口函数*/
{
    ...
    atomic_set(&blockio.keyvalue, INVALKEY);
