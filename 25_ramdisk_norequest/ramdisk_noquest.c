...
void ramdisk_make_request_fn(struct request_queue *q, struct bio *bio)
{
    ...
    bio_for_each_segment(bvec, bio, iter)
    {
        ...
        if (bio_data_dir(bio) == READ) /* 读数据 */
            memcpy(ptr, ramdiskdev.ramdiskbuf + offset, len);
        else if (bio_data_dir(bio) == WRITE) /* 写数据 */
            memcpy(ramdiskdev.ramdiskbuf + offset, ptr, len);
        offset += len;
    }
...
static int __init ramdisk_init(void)
{
    ...
    /*#5.分配并初始化请求队列*/
    ramdiskdev.request_queue = blk_alloc_queue(GFP_KERNEL);
    ...
    /*#6.设置“制造请求”函数*/
    blk_queue_make_request(ramdiskdev.request_queue, ramdisk_make_request_fn);
