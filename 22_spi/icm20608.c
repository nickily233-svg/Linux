...
struct icm20608_dev
{
    ...
    void *private_data;
    int cs_gpio;
    s32 gyro_x_adc;  /* 陀螺仪X 轴原始值 */
    s32 gyro_y_adc;  /* 陀螺仪Y 轴原始值 */
    s32 gyro_z_adc;  /* 陀螺仪Z 轴原始值 */
    s32 accel_x_adc; /* 加速度计X 轴原始值 */
    s32 accel_y_adc; /* 加速度计Y 轴原始值 */
    s32 accel_z_adc; /* 加速度计Z 轴原始值 */
    s32 temp_adc;    /* 温度原始值 */
};
...
static int icm20608_read_onereg(struct icm20608_dev *dev, u8 reg)
{
    int ret = -1;
    u8 data = 0;
...
static int icm20608_write_onereg(struct icm20608_dev *dev, u8 reg, u8 value)
{
    int ret = -1;
    u8 buf = value;
    ret = icm20608_write_regs(dev, reg, &buf, 1);
    return ret;
}
...
struct file_operations icm20608_ops = {
    .owner = THIS_MODULE,
    .open = icm20608_open,
    .read = icm20608_read,
...
static int icm20608_probe(struct spi_device *spi)
{
    ...
    ret = spi_setup(spi);
    printk("spi_setup ret=%d mode=%u bpw=%u speed=%u\r\n", ret, spi->mode, spi->bits_per_word, spi->max_speed_hz);
