...
static ssize_t bmp280_read(struct file *filp, char __user *buf, size_t size,
                           loff_t *offset) {
  ...
  /* 处理原始数据 */
  /* 换算成压强 */
  adc_p = (s32)((raw_data[0]) << 12 | (raw_data[1] << 4) | (raw_data[2] >> 4));
  adc_t = (s32)((raw_data[3] << 12) | (raw_data[4] << 4) | (raw_data[5] >> 4));
  ...
  var1_64 = (((((int64_t)1) << 47) + var1_64)) * ((int64_t)bmp280.dig_P1) >> 33;
  ...
  err = copy_to_user(buf, &P, sizeof(P));
  if (err) {
    printk("%s %s line %d\n", __FILE__, __FUNCTION__, __LINE__);
    return -EFAULT;
  }
